...
import cn.id0755.sdk.android.conf.ConfigEntity;
            ...
            try {
                ...
                if (pFromServer.isQoS()) {
                    if (pFromServer.getType() == ProtocalType.S.FROM_SERVER_TYPE_OF_RESPONSE$LOGIN
                        ...
                        if (QoS4ReceiveDaemon.getInstance(context).hasRecieved(pFromServer.getFp())) {
                            if (ClientCoreSDK.DEBUG)
                                Log.d(TAG, "【IMCORE】【QoS机制】" + pFromServer.getFp()
                                        + "已经存在于发送列表中，这是重复包，通知应用层收到该包罗！");

                            QoS4ReceiveDaemon.getInstance(context).addRecieved(pFromServer);
