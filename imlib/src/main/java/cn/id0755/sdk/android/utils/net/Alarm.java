...
public class Alarm extends BroadcastReceiver {
    ...
    private final static String K_EXTRA_ID = "ID";
    private final static String K_EXTRA_PID = "PID";
    ...
    private static WakeLockUtil wakelock = null;
...
//    private native void onAlarm(final long id);
    ...
    public static void resetAlarm(final Context context) {
        synchronized (alarm_waiting_set) {
            ...
            while (it.hasNext()) {
                cancelAlarmMgr(context, (PendingIntent) (it.next()[TSetData.PENDING_INTENT.ordinal()]));
            }
    ...
    public static boolean start(final long id, final int after, final Context context) {
        ...
        synchronized (alarm_waiting_set) {
            if (null == wakelock) {
                wakelock = new WakeLockUtil(context);
                Log.i(TAG, "start new wakelock");
            }
        ...
        synchronized (alarm_waiting_set) {
            ...
            while (it.hasNext()) {
                ...
                if ((Long) (next[TSetData.ID.ordinal()]) == id) {
                    cancelAlarmMgr(context, (PendingIntent) (next[TSetData.PENDING_INTENT.ordinal()]));
                    it.remove();
                    return true;
                }
            }
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        final Long id = intent.getLongExtra(K_EXTRA_ID, 0);
        final Integer pid = intent.getIntExtra(K_EXTRA_PID, 0);
        ...
        synchronized (alarm_waiting_set) {
            ...
            while (it.hasNext()) {
                ...
                if (curId.equals(id)) {
                    ...
                    Log.i(TAG, "onReceive find alarm id:%d, pid:%d, delta miss time:%d", id, pid, SystemClock.elapsedRealtime() - (Long) next[TSetData.WAIT_TIME.ordinal()]);
                    it.remove();
                    found = true;
                    break;
                }
        ...
        if (null != wakelock) wakelock.lock(200);
//        if (found) onAlarm(id);
    }
}
